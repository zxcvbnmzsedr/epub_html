...
$(function () {
    ...
    $(".editCourse").click(function () {
        ...
        if (id > 0) {
            var course = parseJson($(this).attr("data-course") || "{}")
            var keys = Object.keys(course)
            for (let i = 0; i < keys.length; i++) {
                $("#addCourse [name=" + keys[i] + "]").val(course[keys[i]])
            }
        } else {
            $("#addCourse input").val("")
            $("#addCourse textarea").val("")
        }
        $("#addCourse").find("[name=id]").val(id)
